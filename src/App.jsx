...
// --- EXTRACTED COMPONENTS FOR OPTIMAL RENDER PERFORMANCE ---
...
const Navbar = ({ setView, profile, isDark, setIsDark, user, googleSignIn }) => (
...
const Dashboard = ({ user, profile, handleCreateFolder, folders, setActiveFolder, setView, decks, progress }) => {
...
const FolderView = ({ decks, activeFolder, setView, setActiveDeck, handleEditFolder, deleteItem, progress }) => {
...
const Importer = ({ setView, showAlert, setPendingDeck }) => {
...
const DeckEditor = ({ pendingDeck, setPendingDeck, activeFolder, user, setActiveDeck, handleCreateDeck, setView, showAlert }) => {
...
const DeckDetail = ({ activeDeck, progress, history, setView, setPendingDeck, deleteItem, setActiveQuiz, showAlert }) => {
  const [config, setConfig] = useState({
    mode: 'all', 
    shuffle: false,
    timeMode: 'none', 
    timeLimit: 10, 
    timeScope: 'quiz' // 'quiz' or 'question'
  });
...
const QuizRunner = ({ activeQuiz, setActiveQuiz, activeDeck, progress, saveProgress, saveHistory, setQuizResults, setView }) => {
  ...
  useEffect(() => {
    ...
    const timerId = setInterval(() => {
       ...
       setQuizRemaining(prev => prev > 0 ? prev - 1 : 0);
       setQElapsed(prev => prev + 1);
       setQRemaining(prev => prev > 0 ? prev - 1 : 0);
    }, 1000);
...
const ResultsView = ({ quizResults, setActiveQuiz, setView }) => {
  ...
  return (
    <>
      ...
      <div className={`min-h-screen flex flex-col transition-colors duration-500 font-sans ${isDark ? 'theme-dark dark' : 'theme-light'}`}>
        <Navbar setView={setView} profile={profile} isDark={isDark} setIsDark={setIsDark} user={user} googleSignIn={googleSignIn} />
        
        <main className="p-4 md:p-8 relative flex-1 max-w-7xl mx-auto w-full">
          {view === 'dashboard' && <Dashboard user={user} profile={profile} handleCreateFolder={handleCreateFolder} folders={folders} setActiveFolder={setActiveFolder} setView={setView} decks={decks} progress={progress} />}
          {view === 'folder' && activeFolder && <FolderView decks={decks} activeFolder={activeFolder} setView={setView} setActiveDeck={setActiveDeck} handleEditFolder={handleEditFolder} deleteItem={deleteItem} progress={progress} />}
          {view === 'import' && activeFolder && <Importer setView={setView} showAlert={showAlert} setPendingDeck={setPendingDeck} />}
          {view === 'edit_deck' && pendingDeck && <DeckEditor pendingDeck={pendingDeck} setPendingDeck={setPendingDeck} activeFolder={activeFolder} user={user} setActiveDeck={setActiveDeck} handleCreateDeck={handleCreateDeck} setView={setView} showAlert={showAlert} />}
          {view === 'deck' && activeDeck && <DeckDetail activeDeck={activeDeck} progress={progress} history={history} setView={setView} setPendingDeck={setPendingDeck} deleteItem={deleteItem} setActiveQuiz={setActiveQuiz} showAlert={showAlert} />}
          {view === 'quiz' && activeQuiz && <QuizRunner activeQuiz={activeQuiz} setActiveQuiz={setActiveQuiz} activeDeck={activeDeck} progress={progress} saveProgress={saveProgress} saveHistory={saveHistory} setQuizResults={setQuizResults} setView={setView} />}
          {view === 'results' && quizResults && <ResultsView quizResults={quizResults} setActiveQuiz={setActiveQuiz} setView={setView} />}
        </main>
